...
function test_handler(message, result, expected){
  it([moduleName, message, result].join(": "), function(){
...
// These are for tests which I think are just plain wrong
function bad_test(){}

function eventualy_test_handler(){}
...
var testData = {
    ...
    html: '<a>',
    syntax_error    : function(){ throw SyntaxError('test syntax error');},
    reference_error : function(){ throw ReferenceError('test reference error');},
    type_error      : function(){ throw TypeError('test type error');}
};

var R = function ( tmpl, data ) {
  try {
    return Bluemold( tmpl, data );
  }
  catch ( e ) {
    if ( typeof e === 'string' ) {
      return 'ERROR: ' + e;
...
module("Basics");
  ...
  test("Basic Function", function() {
    ...
    eventualy_test_handler( "_ (underscore) cannot by used by data", R('${ _ }', {'_':'foo'}), TypeError );
    eventualy_test_handler( "$ cannot be used by data", R('${ $ }', {'$':'foo'}), TypeError );

  });

  // test errors are passed back correctly
  test("Error Passing", function(){
    test_handler( 'syntax', R("{{html syntax_error() }}", testData), SyntaxError );
    test_handler( 'reference', R("{{html reference_error() }}", testData), ReferenceError );
    test_handler( 'type', R("{{html type_error() }}", testData), TypeError );
  });

  // newlines should work because: ${ foo + "\n" } and they have whitespace management benefits
  eventually_test("Newlines / Escaping", function(){
  ...
  eventually_test("Ignore Malformed Tags", function() {
  ...
  eventually_test("Caching via $.template() and .template()", function() {
  ...
  test("Variables", function() {
    ...
    // I do not understand the "self" reference
    bad_test( 'empty variable tag (with tabs whitespace)', R("{{=\t\t}}", "self"), 'self' );
    ...
    // This is magic
    bad_test( 'member functions in a sequence don\'t get called', R("${ foo.bar }", cls), '' );
  ...
  test("Falsy Values", function(){
    ...
    test_handler( "(null)", R('${ null }'), '' );   // it's debatable what we want here
...
module("Commands");
  ...
  eventually_test("Create New Command", function(){
  ...
  test("Each {{ each }}", function() {
    ...
    test_handler( 'errors are passed back correctly (syntax)', R("{{each arr}}${ $i }{{html syntax_error()}}{{/each}}", testData), SyntaxError );
  ...
  test("{{if}} and {{else}}", function() {
    ...
    bad_test( "/if ignores following text", R('{{if a}}TRUE{{else}}FALSE{{/if a}}', { a:1 }), 'TRUE' );
  ...
  eventually_test("{{tmpl() template}}", function() {
...
module("Script Tag Caching");
  ...
  eventually_test("Template Reuse", function(){
    var template = $('#reuse'),
      data = {data: 'pass1'};
